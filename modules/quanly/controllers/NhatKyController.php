...
class NhatKyController extends QuanlyBaseController
{
    ...
    public function actionPhanTichTuan($thang = null, $nam = null)
    {
        ...
        $nguoi_pt = $firstRec !== null ? $firstRec->nguoi_pt : '';
        $nguoi_kt = $firstRec !== null ? $firstRec->nguoi_kt : '';
    ...
    public function actionDongHoSave()
    {
        ...
        if (!$model)
            return ['success' => false, 'msg' => 'Không tìm thấy bản ghi'];

        $dvao = $post['channel_dau_vao'] ?? [];
        $dra = $post['channel_dau_ra'] ?? [];
        if (is_string($dvao))
            $dvao = array_filter(array_map('trim', explode(',', $dvao)));
        if (is_string($dra))
            $dra = array_filter(array_map('trim', explode(',', $dra)));
    ...
    public function actionApiSanLuong($tu_ngay = null, $den_ngay = null)
    {
        ...
        $khachHang = NkDongHoKhachHang::getActive();
        if (empty($khachHang))
            return ['success' => true, 'rows' => [], 'ngay_list' => []];

        $allChannels = [];
        foreach ($khachHang as $kh) {
            foreach ($kh->getChannelDauVaoArr() as $cid)
                $allChannels[$cid] = true;
            foreach ($kh->getChannelDauRaArr() as $cid)
                $allChannels[$cid] = true;
        }
