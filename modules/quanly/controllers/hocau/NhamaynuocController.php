        ...
        if ($model->load($request->post()) && $model->save() && $filedinhkem->load($request->post())) {
            ...
            if($filedinhkem->fileupload != null){
                ...
                foreach($filedinhkem->fileupload as $i => $item){
                    ...
                    $file[] = 'uploads/nhamaynuoc/'.$model->id.'/'.$item->baseName.'.'.$item->extension;
                    $path = 'uploads/nhamaynuoc/'.$model->id.'/';
            ...
            Yii::$app->db
                ->createCommand("UPDATE network_nhamaynuoc SET geom = ST_SETSRID(ST_GeomFromText(ST_AsText(ST_GeomFromGeoJSON('" . $model->geojson . "'))),4326) WHERE id = :id")
            ...
            */
            ...
            if ($request->isGet) {
                ...
                return [
                    ...
                    'title' => "Xóa nhà máy nước thành công #" . $id,
