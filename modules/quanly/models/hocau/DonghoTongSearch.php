...
/**
 * DonghoTongSearch represents the model behind the search form about `app\modules\quanly\models\hocau\DonghoTong`.
 */
class DonghoTongSearch extends DonghoTong
{
    /**
     * @inheritdoc
     */
    ...
    {
        return [
            [['id', 'objectid_1', 'objectid', 'co', 'status', 'created_by', 'updated_by', 'tinhtrang_id', 'hieudongho_id'], 'integer'],
    ...
    {
        $query = DonghoTong::find()->where(['status' => 1]);
        ...
        $query->andFilterWhere([
            ...
            'hieudongho_id' => $this->hieudongho_id,
        ]);
        ...
        $query->andFilterWhere(['like', 'upper(geom)', mb_strtoupper($this->geom)])
            ...
            ->andFilterWhere(['like', 'upper(madongho)', mb_strtoupper($this->madongho)])
