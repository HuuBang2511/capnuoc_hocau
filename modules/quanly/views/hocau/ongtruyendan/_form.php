...
<?php 
    if($model->file_dinhkem != null){
        ...
        foreach($model->file_dinhkem as $i => $item){
            $file[] = Url::to(['/quanly/hocau/ongtruyendan/download-file', 'path' => $item]);
        }
    }
?>
...
//                        'language' => 'vn',
                ...
                <div class="col-lg-3">
                    <?= $form->field($model, 'mavattu')->textInput() ?>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-3">
                    <?= $form->field($model, 'loaiong_id')->widget(Select2::className(), [
                        'data' => ArrayHelper::map($categories['loaiong'], 'id', 'ten'),
                        'options' => ['placeholder' => 'Loại ống' ],
                        'pluginOptions' => [
                            'allowClear' => true
                        ],
                    ]) ?>
...
const overlayers = {
    "Ống nước thô": L.tileLayer.wms('https://cello.capnuochocaumoi.vn/geoserver/capnuoc_hocau/wms', {
